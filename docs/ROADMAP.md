...
- [ ] **[V-02]** ODsay 경로 탐색 1,000건/일 한도가 실시간 도착 API와 **통합 한도**인지 **별도 한도**인지 공식 문서 또는 실제 응답 헤더로 확인
- [ ] **[V-03]** ODsay 경로 응답에서 `path[0].subPath` 배열 구조 및 `lane[0].arsID`, `lane[0].busNo`, `startName`, `lane[0].name` 필드 실존 여부 확인
- [ ] **[V-04]** ODsay `realtimeSubway` API 응답에서 `barvlDt`(초 단위) 필드 존재 여부 및 단위 재확인
...
- [ ] **[T-01]** Supabase 프로젝트 생성 및 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` 환경변수 설정 (선행: 없음)
- [ ] **[T-02]** `day_of_week` ENUM 타입 생성 및 `user_settings` 테이블 생성 — `home_address`, `home_lat`, `home_lng`, `buffer_minutes`, `commute_start_hour`, `commute_end_hour`, `TIMESTAMPTZ`, RLS 포함 (선행: T-01)
- [ ] **[T-03]** `schedules` 테이블 생성 — 출근지 컬럼, `commute_traffic_type`, `commute_ars_id`, `commute_bus_no`, `commute_station_name`, `commute_subway_line`, `odsay_route_cache` JSONB, `route_cached_at`, 퇴근 방향 `return_*` 대칭 컬럼 전체, RLS, `UNIQUE(user_id, day)` (선행: T-01)
- [ ] **[T-04]** `overrides` 테이블 생성 — `override_date` DATE, 출근/퇴근 방향 `commute_*` / `return_*` 컬럼, `route_cached_at` / `return_route_cached_at` (당일 중복 탐색 방지 목적), RLS, `UNIQUE(user_id, override_date)` (선행: T-01)
- [ ] **[T-05]** 인덱스 생성 — `idx_schedules_user_day ON schedules(user_id, day)`, `idx_overrides_user_date ON overrides(user_id, override_date)` (선행: T-03, T-04)

**인증**

- [ ] **[T-06]** `lib/supabase/client.ts`, `lib/supabase/server.ts` 구현 (선행: T-01)
- [ ] **[T-07]** `app/layout.tsx` 또는 최상위 Provider에 `onAuthStateChange` 리스너 등록 — `SIGNED_OUT` 이벤트 발생 시에만 `signInAnonymously()` 재호출, `TOKEN_REFRESHED` 이벤트는 무시 (선행: T-06)
- [ ] **[T-08]** 앱 최초 접속 시 세션 없는 경우 백그라운드 `signInAnonymously()` 자동 호출 로직 구현 (선행: T-07)

**환경변수**

- [ ] **[T-09]** `.env.local` 파일 설정 — `KAKAO_REST_API_KEY`, `ODSAY_API_KEY` 추가, `.env.example` 업데이트 (선행: T-01)

**Route Handler 뼈대**

- [ ] **[T-10]** `app/api/kakao/address/route.ts` 구현 — `GET ?query=`, 카카오 로컬 API 프록시, 5초 타임아웃, `export const dynamic = 'force-dynamic'` (선행: T-09)
- [ ] **[T-11]** `app/api/odsay/route/route.ts` 구현 — `POST { sx, sy, ex, ey }`, ODsay 경로 탐색 프록시, 응답에서 `trafficType` 분기 후 `ars_id`/`bus_no`/`station_name`/`subway_line`/`total_time`/`full_cache` 추출 반환, 5초 타임아웃 (선행: T-09, V-01 검증 결과 반영)
- [ ] **[T-12]** `app/api/transit/arrival/route.ts` 구현 — `GET ?type=bus&arsId=&busNo=` 및 `?type=subway&stationName=&line=` 분기 처리, ODsay 실시간 API 프록시, 5초 타임아웃 (선행: T-09, V-02, V-03 검증 결과 반영)
...
- [ ] **[T-13]** `lib/utils.ts`에 KST 시간 유틸리티 추가 — `toKST(date)`, `getKSTHour()`, `getKSTDate()` (date-fns-tz 활용), `arrival_time`(TIME 문자열)과 현재 KST 시각 비교 함수 (선행: 없음)
- [ ] **[T-14]** `lib/odsay.ts` 구현 — `/api/odsay/route` 호출 함수, `schedules` 테이블 캐시 유효성 검사(`odsay_route_cache IS NULL` OR `route_cached_at < 7일 전`) 포함, Supabase 업데이트까지 처리 (선행: T-11)
- [ ] **[T-15]** `lib/transit.ts` 구현 — `/api/transit/arrival` 호출 함수, `commute_traffic_type` 기반 분기, 응답 정규화 (선행: T-12)
- [ ] **[T-16]** `lib/kakao.ts` 구현 — `/api/kakao/address` 호출 함수, 디바운스 처리 포함 (선행: T-10)
- [ ] **[T-16M]** `lib/mock/transitMock.ts` Mock Data 유틸리티 생성 — ODsay 버스/지하철 API 응답 형태와 동일한 가짜 데이터 객체 및 반환 함수 구현. 목적: UI/CSS 작업 시 실제 API를 호출하지 않고 `useTransitInfo` 훅과 동일한 인터페이스로 화면에 데이터를 렌더링하여 ODsay 일일 한도(1,000건) 소진 방지. 버스 Mock(`mockBusArrival`), 지하철 Mock(`mockSubwayArrival`), 경로 캐시 Mock(`mockOdsayRouteCache`)을 각각 구현하며 실제 API 응답 필드명(`routeNm`, `predictTime1`, `locationNo1`, `barvlDt` 등)과 100% 동일하게 맞출 것 (선행: V-01~V-03 완료 후 실제 응답 구조 확인 기반으로 작성)

**공통 훅**

- [ ] **[T-17]** `hooks/useSchedule.ts` 구현 — 오늘 KST 날짜 및 요일 기준으로 `overrides` 우선 조회 → 없으면 `schedules` 조회, `odsay_route_cache` 유효성 검사 후 만료 시 `lib/odsay.ts` 호출하여 캐시 갱신 트리거, `commute_traffic_type = NULL`이면 탐색 중 상태 반환 (선행: T-13, T-14)
- [ ] **[T-18]** `hooks/useTransitInfo.ts` 구현 — TanStack Query `useQuery`, `refetchInterval: 30000`, `retry: 1`, `commute_traffic_type` 기반 버스/지하철 API 분기, 연속 3회 에러 시 폴링 중지 + "네트워크 오류" 상태 반환, 세션 복구 중 폴링 일시 중단 (선행: T-15, T-17)
- [ ] **[T-19]** `hooks/useDepartureTimer.ts` 구현 — `arrival_time`(KST TIME) - `odsay_route_cache.info.totalTime`(분) - `buffer_minutes` 계산으로 출발 권장 시각 산출, 1분 단위 카운트다운, 이미 지난 경우 "서둘러 출발하세요!" 반환, `commute_traffic_type = NULL`이면 null 반환 (선행: T-13, T-17)

**공통 컴포넌트**

- [ ] **[T-20]** `components/schedule/AddressSearch.tsx` 구현 — 카카오 주소 검색 입력 + 결과 목록, `x`(lng)/`y`(lat) 좌표 반환 콜백, 300ms 디바운스, 시니어 친화 (48px 터치 타겟, 18px 이상 폰트) (선행: T-16)
- [ ] **[T-21]** `components/schedule/TimeInput.tsx` 구현 — HH:MM 시간 입력 피커, 큰 터치 타겟, 한국어 레이블 (선행: 없음)
- [ ] **[T-22]** `components/schedule/DaySelector.tsx` 구현 — 월~금 요일 선택 + `is_active` 토글, 한국어 표시 (선행: 없음)
- [ ] **[T-23]** `components/commute/ModeHeader.tsx` 구현 — 출근/퇴근 모드 헤더, 현재 모드 표시 (선행: 없음)

**TanStack Query 설정**

- [ ] **[T-24]** `components/providers/` 또는 `app/layout.tsx`에 `QueryClientProvider` 설정, 기본 `retry: 1` 설정 (선행: T-06)
...
- [ ] **[T-25]** `app/setup/page.tsx` 레이아웃 구성 — 집 주소 섹션 + 요일별 스케줄 섹션, 단일 화면 기반 (선행: T-22, T-21, T-20)
- [ ] **[T-26]** 집 주소 등록/수정 UI — `AddressSearch` 컴포넌트 연결, 저장 시 `user_settings` upsert (`home_lat`, `home_lng`, `home_address`), `buffer_minutes` 및 모드 전환 시각 설정 포함 (선행: T-20, T-02)
- [ ] **[T-27]** 요일별 스케줄 등록 UI — `DaySelector` + `AddressSearch` + `TimeInput` 조합, `is_active` 토글, 저장 시 `schedules` upsert (선행: T-22, T-20, T-21, T-03)
- [ ] **[T-28]** 스케줄 목록 조회 및 삭제 UI — 등록된 요일별 스케줄 카드 표시, 삭제 시 `schedules` 레코드 삭제 + 연결된 `overrides` 확인 (선행: T-27)
- [ ] **[T-29]** 스케줄 없는 초기 상태에서 설정 화면으로 유도하는 빈 상태 UI (선행: T-25)
- [ ] **[T-30]** 초기 설정 화면에 "이 앱의 데이터는 이 기기에 저장됩니다. 브라우저 데이터를 삭제하면 스케줄을 다시 등록해야 합니다" 안내 문구 (선행: T-25)

**F-02 — 출근 모드 메인 화면**

- [ ] **[T-31]** `app/page.tsx` KST 시간 기반 모드 판별 로직 — `commute_start_hour <= KST 현재 시각 < commute_end_hour`이면 출근 모드, `commute_end_hour <= KST 현재 시각`이면 퇴근 모드 (선행: T-13, T-17)
- [ ] **[T-32]** 출근 모드 메인 화면 레이아웃 — 출근지명(32px 이상), 목표 도착 시간(32px 이상), 타이머 섹션, 실시간 대중교통 섹션, 한 화면에 최대 3개 주요 정보 (선행: T-23, T-31)
- [ ] **[T-33]** 오늘 요일에 스케줄 없는 경우 처리 — 주말: "오늘은 쉬는 날이에요", `is_active = false`: "오늘은 휴무일로 설정되어 있어요", 스케줄 미등록: "아직 오늘 스케줄이 없어요. 등록하러 가기" 버튼 (선행: T-17)
- [ ] **[T-34]** `useSchedule` 연동 및 오버라이드 배지 표시 — 오버라이드 활성 시 "오늘만 변경된 일정입니다" 배지 (선행: T-17)

**F-03 — 출발 타이머**

- [ ] **[T-35]** `components/commute/DepartureTimer.tsx` 구현 — **[Mock Data 우선 원칙]** UI/CSS 작업 중 잦은 핫리로딩으로 ODsay 일일 한도(1,000건)가 소진되는 것을 방지하기 위해, 반드시 `mockOdsayRouteCache`(T-16M)를 먼저 prop으로 주입하여 화면과 반응형 UI를 100% 완성한 뒤, 마지막 단계에서 `useDepartureTimer` 훅으로 교체한다. "N분 뒤 출발하세요" (32px 이상), 이미 지난 경우 "서둘러 출발하세요!" 경고, `commute_traffic_type = NULL`이면 "경로를 탐색 중입니다" 표시 (선행: T-16M, T-19)
- [ ] **[T-36]** 타이머 1분 단위 카운트다운 인터벌 구현, 앱 포어그라운드 복귀 시 재동기화 (선행: T-35)

**F-04 — 실시간 대중교통 정보**

- [ ] **[T-37]** `components/commute/TransitInfo.tsx` 구현 — **[Mock Data 우선 원칙]** UI/CSS 작업 중 잦은 핫리로딩으로 ODsay 일일 한도(1,000건)가 순식간에 소진되는 것을 방지하기 위해, 반드시 `mockBusArrival` / `mockSubwayArrival`(T-16M)을 먼저 prop으로 주입하여 버스/지하철 분기 렌더링과 반응형 UI를 100% 완성한 뒤, 마지막 단계에서 `useTransitInfo` 훅으로 교체한다 (선행: T-16M, T-18)
- [ ] **[T-38]** 버스 정보 렌더링 — Mock Data 기반으로 정류장명, 노선 번호, "버스 도착까지 N분", 남은 정류장 수 (1~2순위) UI 완성 후 실제 훅 연결. 18px 이상 폰트, 한국어 풀어쓰기 (선행: T-37)
- [ ] **[T-39]** 지하철 정보 렌더링 — Mock Data 기반으로 역명, 노선명, 행선지, "도착까지 N분"(barvlDt 초 단위 → 분 변환) UI 완성 후 실제 훅 연결. 18px 이상 폰트 (선행: T-37)
- [ ] **[T-40]** Fallback UI 구현 — 타임아웃: "현재 도착 정보를 불러오지 못했습니다. 잠시 후 다시 시도합니다", 빈 응답: "현재 운행 정보 없음", `traffic_type = NULL`: "경로를 탐색 중입니다", 연속 3회 에러: "네트워크 오류" + 폴링 중지 안내 (선행: T-37)
- [ ] **[T-41]** ODsay 경로 탐색 결과 `schedules` 테이블 자동 업데이트 — `useSchedule` 캐시 만료 감지 후 `/api/odsay/route` 호출 → 응답 파싱 → Supabase `schedules` upsert (선행: T-14, T-17)
...
- [ ] **[T-42]** 퇴근 모드 레이아웃 구현 — **[Mock Data 우선 원칙]** `mockBusArrival` / `mockSubwayArrival`(T-16M)로 집 방향 대중교통 도착 정보 및 예상 귀가 시간 UI를 100% 완성한 뒤 실제 훅으로 교체. `ModeHeader` 퇴근 모드 표시 포함 (선행: T-16M, T-31, T-23)
- [ ] **[T-43]** 퇴근 모드에서 `useTransitInfo` 훅 재활용 연결 — T-42 UI 완성 후 진행. 오버라이드 활성 시 `overrides.return_*` 우선, 없으면 `schedules.return_*` 사용, `return_traffic_type` 기반 버스/지하철 분기 (선행: T-18, T-42)
- [ ] **[T-44]** 퇴근 방향 ODsay 경로 탐색 자동 실행 — `schedules.return_route_cached_at`이 없거나 7일 초과 시 `lib/odsay.ts`로 직장→집 방향 탐색, `return_*` 컬럼 업데이트 (선행: T-14)
- [ ] **[T-45]** 당일 출근지 오버라이드가 있는 날 퇴근 모드 — `overrides.return_*`가 NULL이면 `schedules.return_*` 재활용 (시간만 변경한 오버라이드 케이스), ODsay 재탐색 없이 기존 캐시 사용 (선행: T-43)

**F-06 — 오버라이드**

- [ ] **[T-46]** `app/override/page.tsx` 구현 — "목표 시간만 변경" / "출근지 변경" / "전부 변경" 3가지 선택지 UI, 큰 버튼(48px 이상 터치 타겟) (선행: T-20, T-21)
- [ ] **[T-47]** "목표 시간만 변경" 처리 — `arrival_time` 오버라이드 저장, `overrides.return_*`은 NULL 유지 (퇴근 시 `schedules.return_*` 재활용), ODsay 재탐색 없음 (선행: T-04, T-46)
- [ ] **[T-48]** "출근지 변경" 처리 — `AddressSearch`로 새 목적지 선택, `overrides.route_cached_at`이 당일(`CURRENT_DATE KST 기준`)인 경우 ODsay 재탐색 건너뛰고 기존 `odsay_route_cache` 재사용, 아닌 경우 출근 방향(`집→변경 출근지`) + 퇴근 방향(`변경 출근지→집`) ODsay 동시 탐색 후 `overrides.commute_*` + `overrides.return_*` 동시 저장 (선행: T-11, T-04, T-46)
- [ ] **[T-49]** 오버라이드 저장 완료 후 메인 화면 복귀 및 `useSchedule` 캐시 무효화, "오늘만 변경된 일정입니다" 배지 표시 (선행: T-34, T-47, T-48)
- [ ] **[T-50]** 오버라이드 취소 기능 — `overrides` 레코드 삭제 후 기존 스케줄로 복원 (선행: T-49)

**Fallback UI 완성**

- [ ] **[T-51]** ODsay 경로 탐색 타임아웃 Fallback — "경로 정보를 불러오지 못했습니다. 타이머를 표시할 수 없습니다" + 재시도 버튼, `commute_traffic_type = NULL` 유지 (선행: T-11)
- [ ] **[T-52]** ODsay 한도 초과 Fallback — 기존 캐시 사용, 캐시도 없으면 목표 도착 시간만 표시 (선행: T-40)
- [ ] **[T-53]** 사용자가 화면 탭 또는 앱 포어그라운드 전환 시 폴링 재개 로직 (`visibilitychange` 이벤트 활용) (선행: T-18)
