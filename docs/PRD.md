...
| 트리거 | 앱 실행 시각이 `commute_start_hour` 이후이고, `arrival_time`(목표 도착 시각)에 아직 도달하지 않은 경우. `arrival_time`이 없으면 `commute_end_hour` 미만인 경우 출근 모드 (기본 오전 5:00 이후) |
...
|------|-----------|
| 기능명 | 퇴근 모드 자동 전환 |
| 설명 | 현재 시각이 `arrival_time`(목표 도착 시각) 이상이 되면 자동으로 퇴근 모드로 전환. `arrival_time`이 없는 경우 `commute_end_hour` 이후를 fallback 기준으로 사용. 당일 출근지 오버라이드가 있는 경우, 변경된 출근지를 퇴근 출발지로 사용. 오버라이드가 없으면 `schedules`의 `return_traffic_type` 기반으로 집 방향 실시간 도착 정보 표시 (F-04 로직 재활용) |
| 트리거 | 현재 시각 ≥ `arrival_time` (KST 기준). `arrival_time` 미설정 시 fallback: 현재 시각 ≥ `commute_end_hour` |
...
[Supabase Anonymous Auth 확인]
  ...
  세션 이벤트 이상 → onAuthStateChange에서 signInAnonymously() 자동 재호출
        │
        ▼
[현재 시각 체크 — getDirectionByArrivalTime()]
  arrival_time 있음: 현재 시각 < arrival_time → 출근 모드 / 현재 시각 ≥ arrival_time → 퇴근 모드
  arrival_time 없음(fallback): 현재 시각 < commute_end_hour → 출근 모드 / 이후 → 퇴근 모드
  (return_* 컬럼 사용)
        │ (출근 모드 기준)
        ▼
[Supabase 조회]
